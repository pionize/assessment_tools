...
describe('API Service', () => {
  ...
  describe('authenticateCandidate', () => {
    it('should authenticate candidate and return candidate and assessment data', async () => {
      ...
      expect(result.candidate.email).toBe('john@example.com')
      expect(result.candidate.assessmentId).toBe('assessment-123')
      expect(result.assessment.id).toBe('assessment-123')
    })

    it('should throw error for invalid assessment ID', async () => {
      await expect(
        apiService.authenticateCandidate('invalid-id', 'John Doe', 'john@example.com')
  ...
  describe('getChallenges', () => {
    ...
    it('should throw error for invalid assessment ID', async () => {
      await expect(
        apiService.getChallenges('invalid-id')
      ).rejects.toThrow('Assessment not found')
    })
  })
  ...
  describe('submitChallenge', () => {
    ...
    it('should validate code submission has files', async () => {
      ...
      await expect(
        apiService.submitChallenge(submission)
      ).rejects.toThrow('Code submission must include files')
    })
    ...
    it('should validate multiple choice submission has answers', async () => {
      ...
      await expect(
        apiService.submitChallenge(submission)
      ).rejects.toThrow('Multiple choice submission must include answers')
    })
    ...
    it('should validate open-ended submission has answer', async () => {
      ...
      await expect(
        apiService.submitChallenge(submission)
      ).rejects.toThrow('Open-ended submission must include an answer')
    })
  })
