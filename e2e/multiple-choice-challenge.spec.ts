...
test.describe('Multiple Choice Challenge', () => {
  ...
  test('should display challenge content', async ({ page }) => {
    ...
    await expect(page.getByText('Test your knowledge of JavaScript fundamentals')).toBeVisible()
    await expect(page.getByText('Answer the following multiple-choice questions')).toBeVisible()
  })

  test('should show questions and options', async ({ page }) => {
    await expect(page.getByText('What is the output of `console.log(typeof null);`?')).toBeVisible()
  ...
  test('should enable submit when all questions answered', async ({ page }) => {
    ...
    for (let i = 0; i < questions; i++) {
      const questionOptions = page.locator('.challenge-question').nth(i).locator('input[type="radio"]')
  ...
  test('should show confirmation for incomplete submission', async ({ page }) => {
  ...
  test('should navigate back to challenge list after submission', async ({ page }) => {
    // Answer all questions
    const questions = await page.locator('[data-testid="question"]').count()
    
    for (let i = 0; i < questions; i++) {
      const questionOptions = page.locator('[data-testid="question"]').nth(i).locator('input[type="radio"]')
      await questionOptions.first().click()
    }
  ...
  test('should show different styles for correct/incorrect in results', async ({ page }) => {
